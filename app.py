...
class InsightEngine:
    """Generate business intelligence from invoice data - NO API CALLS"""
    ...
    def __init__(self, user_id):
        ...
        self.last_year = this_year if self.this_month > 1 else this_year - 1
    ...
    def _revenue_insight(self):
        this_month = [i for i in self.invoices 
                      if i.issue_date.month == self.this_month 
                      and i.issue_date.year == self.this_year 
                      and i.status == 'Paid']
        last_month = [i for i in self.invoices 
                      if i.issue_date.month == self.last_month 
                      and i.issue_date.year == self.last_year 
                      and i.status == 'Paid']
        
        this_revenue = sum(i.total for i in this_month)
        last_revenue = sum(i.total for i in last_month)
        
        if last_revenue == 0:
            change_pct = 100 if this_revenue > 0 else 0
            trend = 'up' if this_revenue > 0 else 'flat'
        else:
            change_pct = ((this_revenue - last_revenue) / last_revenue) * 100
        ...
        return {
            'type': 'revenue',
            'icon': icon,
            'title': f"Revenue {'increased' if trend == 'up' else 'decreased' if trend == 'down' else 'remained flat'} by {abs(change_pct):.1f}%",
            'description': f"This month: RM {this_revenue:,.2f} | Last month: RM {last_revenue:,.2f}",
            'action': 'Keep up the momentum!' if trend == 'up' else 'Review pricing strategy' if trend == 'down' else 'Maintain current pace',
            'color': color,
    ...
    def _payment_insight(self):
        if not self.invoices:
            return {'type': 'payment', 'icon': '❌', 'title': 'No invoices', 'description': '', 'color': 'gray', 'action': '', 'metric': '0%'}
        ...
        if paid_pct >= 80:
            status, action, color = 'Excellent', 'Your payment collection is outstanding!', 'green'
        elif paid_pct >= 60:
            status, action, color = 'Good', 'Keep following up on unpaid invoices', 'blue'
        elif paid_pct >= 40:
            status, action, color = 'Fair', 'Focus on payment collection', 'yellow'
        else:
            status, action, color = 'Needs Attention', 'Urgent: Follow up on overdue invoices', 'red'
        
        return {
            'type': 'payment',
            'icon': '⏰',
            'title': f'Payment Health: {status}',
            'description': f'{paid}/{total} invoices ({paid_pct:.1f}%) have been paid',
            'action': action,
            'color': color,
            'metric': f"{paid_pct:.0f}%"
        }
    ...
    def _client_insight(self):
        ...
        for inv in self.invoices:
            if inv.status == 'Paid':
                client_revenue[inv.client_name] += inv.total
        
        if not client_revenue:
            return {'type': 'client', 'icon': '👤', 'title': 'No paid invoices yet', 'description': '', 'color': 'gray', 'action': '', 'metric': 'RM 0'}
        
        top_client = max(client_revenue.items(), key=lambda x: x[1])
        total_revenue = sum(client_revenue.values())
        client_pct = (top_client[1] / total_revenue * 100)
        
        return {
            'type': 'client',
            'icon': '⭐',
            'title': f'{top_client[0]}',
            'description': f'Generated RM {top_client[1]:,.2f} ({client_pct:.0f}% of revenue)',
            'action': 'Nurture this relationship',
            'color': 'purple',
            'metric': f"RM {top_client[1]:,.0f}"
        }
    ...
    def _overdue_insight(self):
        ...
        if overdue_count == 0:
            return {
                ...
                'action': 'Excellent cash flow management!',
                'color': 'green',
                'metric': '0'
            }
        ...
        return {
            ...
            'description': f'Total amount: RM {overdue_total:,.2f}',
            'action': 'Send payment reminders immediately',
            'color': 'red',
            'metric': f"RM {overdue_total:,.0f}"
        }
    
    def _service_insight(self):
        this_month_invoices = [i for i in self.invoices 
                               if i.issue_date.month == self.this_month 
                               and i.issue_date.year == self.this_year]
        ...
        if not services:
            return {'type': 'service', 'icon': '🏷️', 'title': 'No services this month', 'description': '', 'color': 'gray', 'action': '', 'metric': '×0'}
        ...
        return {
            ...
            'icon': '🏷️',
            'title': most_common[0],
            'description': f'Billed {most_common[1]} time{"s" if most_common[1] > 1 else ""} this month',
            'action': 'Your specialty – focus on quality',
            'color': 'teal',
            'metric': f"×{most_common[1]}"
        }
...
def get_smart_insights():
    """Generate business intelligence insights (no API calls)"""
    engine = InsightEngine(current_user.id)
    data = engine.get_all_insights()
    return jsonify(data)
